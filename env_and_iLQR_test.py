...
ini_cond_path = "./ini_cond_hard_2.csv"
Using_EKF = 1
Set_ini_theta_B_zero = 1 # non-zero for Syntec report section 5.2
ep_num = 20
...
def run_dynamic_model( x_Bx, x_By, theta_B_, t_duration, x_mB_0 ):
    ...
    for idx in range( itr_num - 1 ):
        ...
        theta_B[idx+1] = theta_B_iLQR[idx+1] + theta_B_PID[idx+1]
...
## variable space initialization ##
itr_num = len( x_Bx )
t_axis = np.arange( 0, itr_num*dt - 1e-10, dt )
x_mB, v_mB, a_mB =  np.zeros( ( 3, itr_num ), dtype=float ) # [mm], [mm/sec], [mm/sec^2]
...
#        msr_tau[idx+1] = tau[idx+1] * np.random.normal( 1., 0.03 )
    ...
    theta_B_set.append( np.copy(theta_B) )
    x_mB_set.append( np.copy(x_mB) )
