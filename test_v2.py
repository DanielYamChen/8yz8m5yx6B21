...
for idx in range(itr_num-1):
    ...
    x_mB[idx+1] = x_mB[idx] + v_mB[idx] * dt
    ...
    # problematic #
#    a_m[idx+1,:] = acc_at_m[:,0] + a_mB[idx] * np.array([ np.cos(theta_B[idx]) , np.sin(theta_B[idx]) ])
#    v_m_[idx+1,:] = v_m_[idx,:] + a_m[idx,:] * dt
#    x_m_[idx+1,:] = x_m_[idx,:] + v_m_[idx,:] * dt
