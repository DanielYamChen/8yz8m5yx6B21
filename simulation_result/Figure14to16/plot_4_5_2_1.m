...
for i = 1 : length( model_idx )
    for j = 1 : segment_num
        temp = csvread( strcat( 'data/GD_process(', num2str( model_idx(i) ), ')_' , path_idx , '.csv' ) ); % [mm]
...
% for j = 1 : 3
%         
%     plot( reshape( GD_process_set( order( j ) , argmin , : ) , [] , 1 ) , reshape( J2_process_set( order( j ) , argmin , : ) , [] , 1 ) , line_style( j ) , 'LineWidth' , 3 );
%     hold on;
%     scatter( GD_process_set( order( j ) , argmin , 1 ) , J2_process_set( order( j ) , argmin , 1 ), 60 , 'filled' );
%     
% end
