...
for i = 1 : segment_num
    ...
    GD_process_set( i , : , : ) = temp( i * 2 - 1 : i * 2 , : );
end
...
%     for j = 1 : segment_num
%         via_points( 1 , 2:3 ) = GD_process_set( j , : , idx_set( i ) );
...
%         
%         via_points( 1 , 2:3 ) = GD_process_set( j , : , 1 );
...
%% PART 2
for j = 1 : segment_num
    plot3( reshape( GD_process_set( j , 1 , : ) , [] , 1 ) , reshape( GD_process_set( j , 2 , : ) , [] , 1 ) , reshape( J2_process_set( j , arg_min , : ) , [] , 1 ) , 'LineWidth' , 3 );
    hold on;
    scatter3( GD_process_set( j , 1 , 1 ) , GD_process_set( j , 2 , 1 ) , J2_process_set( j , arg_min , 1 ) , 60 , 'filled' );
end
...
%%%PART 4
% via_points = csvread( strcat( 'data/best_ini_cond(' , num2str( model_idx( arg_min ) ) ,')_1_.csv' ) , 0 , 0 , [ 0 , 0 , 7 , 0 ] )';
% via_points = reshape( via_points, [], 2 )';
% x_Bx = via_points( 1 , : );
% x_By = via_points( 2 , : );
...
% scatter( x_Bx , x_By , 52, 'filled', 'MarkerFaceColor' , 'k' );
% axis equal;
